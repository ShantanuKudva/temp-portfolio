...
export function LoaderCurtain() {
  ...
  const value = useSpring(target, { stiffness: 80, damping: 22, restDelta: 0.01 });
  const scaleX = useTransform(value, [0, 100], [0, 1]);
  const pct = useTransform(value, (v) =>
  ...
  useEffect(() => {
    ...
    const controls = animate(target, 100, { duration: 2.4, ease: EASE });
  ...
  }, [gone, revealed, target, value, reveal]);
